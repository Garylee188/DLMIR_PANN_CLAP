...
def forward(model, generator, return_input=False, 
    ...
    for batch_data_dict in tqdm(generator):
        # print(n)
        batch_waveform = move_data_to_device(batch_data_dict['waveform'].float(), device)
        
        with torch.no_grad():
            model.eval()
            batch_output = model(batch_waveform)

        # append_to_dict(output_dict, 'audio_name', batch_data)
