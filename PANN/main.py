...
from utils.utilities import (create_folder, get_filename, create_logging, Mixup, 
    StatisticsContainer)
from models import *
from utils.pytorch_utils import (move_data_to_device, count_parameters, count_flops, 
    do_mixup)
...
def train(args):
    ...
    # !!!!!! Speed training. To handle large dataset
    ...
    # Model
    # Model = eval(model_type) # Return the model'name
    # model = Assembly_model(sample_rate=sample_rate, window_size=window_size, 
    #     hop_size=hop_size, mel_bins=mel_bins, fmin=fmin, fmax=fmax, 
        # classes_num=class_num,device=device)
    model = Cnn14(sample_rate=sample_rate, window_size=window_size, 
        hop_size=hop_size, mel_bins=mel_bins, fmin=fmin, fmax=fmax, 
        classes_num=class_num)
    # model = ResNet54_chroma(mel_bins=mel_bins, classes_num=class_num)
    ...
    params_num = count_parameters(model) # Num of parameter
    ...
    # dataset = AudioSetDataset(sample_rate=sample_rate) # !!!!!! modified here
    train_dataset = EmoDataset(song_dir,"20_train.csv")
    val_dataset = EmoDataset(song_dir,"20_val.csv")
    test_dataset = EmoDataset(song_dir,"20_test.csv")


    # Train sampler
    # !!!! Are used to speed up traning
    # if balanced == 'none':
    ...
    eval_bal_loader = torch.utils.data.DataLoader(dataset=val_dataset,
                                                  batch_size=batch_size,
                                               num_workers=num_workers,
                                               pin_memory=True)
    ...
    for each_epoch in range(epochs):
        ...
        for batch_data_dict in pbar:
            ...
            # print(batch_data_dict['waveform'].device)
            # input_for_chorma = \
            #                     torch.tensor(librosa.feature.chroma_stft(
            #                         y=batch_data_dict['waveform'].detach().cpu().numpy(), sr=sample_rate,n_fft=window_size,
            #                         hop_length=hop_size, win_length=window_size, window='hann',
            #                         center=True, pad_mode='reflect',n_chroma=mel_bins)
            #                         ) # (batch,chorma_n, time)
            # input_for_chorma = move_data_to_device(input_for_chorma,device).float()
            # print(input_for_chorma.device)
            ...
            if (iteration % 2000 == 0 and iteration > resume_iteration):# or (iteration == 0):
                ...
                logging.info('Validate bal Accuracy: {:.3f}'.format(
                    np.mean(bal_statistics)))

                logging.info('Validate test Accuracy: {:.3f}'.format(
            ...
            if 'mixup' in augmentation:
                ...
                batch_output_dict = model(batch_data_dict['waveform'].float(), None)
                """{'clipwise_output': (batch_size, classes_num), ...}"""
            ...
            # print(batch_output_dict.shape)
            # print(torch.argmax(batch_output_dict['clipwise_output'],dim=1))
            batch_data_dict['target'] = move_data_to_device(batch_data_dict['target'], device)
            loss = criterion(batch_output_dict['clipwise_output'],batch_data_dict['target'])
            
            
            pbar.set_description(f"Loss:{loss.float()}")
            # print(loss)
            # Backward
            loss.backward()
        ...
        bal_statistics = evaluator.evaluate(eval_bal_loader)
        test_statistics = evaluator.evaluate(eval_test_loader)
        logging.info("{}'s Validate bal Accuracy: {:.3f}".format(
            each_epoch+1,np.mean(bal_statistics)))
        logging.info("{}'s Validate test Accuracy: {:.3f}".format(
            each_epoch+1,np.mean(test_statistics)))
...
if __name__ == '__main__':
    ...
    parser_train.add_argument('--model_type', type=str, default='ResNet54')
    ...
    parser_train.add_argument('--song_dir', type=str,default='emotion')
    parser_train.add_argument('--class_num', type=int, default=190)
    parser_train.add_argument('--epoch', type=int, default=40)
